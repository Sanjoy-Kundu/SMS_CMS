...
class AcademicSectionController extends Controller
{
     ...
     */
    public function academicSectionDetails()
    {
        try {
            $admin = Admin::where('user_id', auth()->id())->first();
            if ($admin) {
                $academicSections = AcademicSection::where('admin_id', $admin->id)
                    ->with('institution') // Institution মডেলের সাথে রিলেশন
                    ->get();
    ...
    public function academicSectionTrash(Request $request)
    {
        ...
        try {
            ...
            return response()->json(
                [
                    'status' => 'error',
                    'message' => 'Failed to trash academic section.',
                    'error' => $e->getMessage(),
                ],
                500,
            );
    ...
    public function academicSectionEditById(Request $request)
    {
        try {
            ...
            $admin = Admin::where('user_id', auth()->id())->first();
            $academicSection = AcademicSection::with('institution')
                ->where('id', $request->id)
                ->where('admin_id', $admin->id)
                ->first();
    ...
    public function academicSectionUpdate(Request $request)
    {
        try {
            ...
            $admin = Admin::where('user_id', auth()->id())->first();
            $academicSection = AcademicSection::where('id', $request->id)
                ->where('admin_id', $admin->id)
                ->first();
